#!/bin/bash
# Test: org-public-repos
# Fetch django org from GitHub API, check public_repos within ±5.
...
print(f"GitHub API says django public_repos: {actual}")
...
if any(abs(n - actual) <= 5 for n in numbers):
    closest = min(numbers, key=lambda n: abs(n - actual))
    pass_test(f"Found {closest} within ±5 of actual {actual}")
else:
    fail(f"No number within ±5 of actual {actual} in {numbers}")
PYEOF
