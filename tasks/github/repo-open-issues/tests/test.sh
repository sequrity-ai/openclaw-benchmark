#!/bin/bash
# Test: repo-open-issues
# Fetch golang/go from GitHub API, check open_issues_count within ±500.
...
try:
    actual = int(data["open_issues_count"])
except Exception as e:
    fail(f"Could not parse API response: {e}")

print(f"GitHub API says golang/go open_issues_count: {actual}")
...
if any(abs(n - actual) <= 500 for n in numbers):
    closest = min(numbers, key=lambda n: abs(n - actual))
    pass_test(f"Found {closest} within ±500 of actual {actual}")
else:
    fail(f"No number within ±500 of actual {actual} in {numbers}")
PYEOF
